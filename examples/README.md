...
Each file maps to the [testnet.okei.fun](https://testnet.okei.fun/) launch form.
...
If the gateway URL loads but testnet.okei.fun shows initials, the site’s IPFS gateway may be timing out. Relaunch with `BUNDLER_LOGO_IMAGE_HTTPS=1` in `.env` so the token stores a direct `https://…/ipfs/…` image link (still valid on okei.fun).
